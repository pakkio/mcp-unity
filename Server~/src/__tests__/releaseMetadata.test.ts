...
describe('release 1.5.2 metadata', () => {
  it('publishes 1.5.2 from both package manifests and the MCP protocol', () => {
    ...
    expect(unityPackage.version).toBe('1.5.2');
    expect(unityPackage.unity).toBe('2022.3');
    expect(nodePackage.version).toBe('1.5.2');
    expect(lockfile.version).toBe('1.5.2');
    expect(lockfile.packages[''].version).toBe('1.5.2');
    expect(serverSource).toContain('version: "1.5.2"');
    expect(dashboardSource).toContain("appInfo: { name: 'unity-dashboard', version: '1.5.2' }");
  });
