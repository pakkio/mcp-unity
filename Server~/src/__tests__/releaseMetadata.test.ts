...
describe('release 1.5.1 metadata', () => {
  it('publishes 1.5.1 from both package manifests and the MCP protocol', () => {
    ...
    expect(unityPackage.version).toBe('1.5.1');
    expect(unityPackage.unity).toBe('2022.3');
    expect(nodePackage.version).toBe('1.5.1');
    expect(lockfile.version).toBe('1.5.1');
    expect(lockfile.packages[''].version).toBe('1.5.1');
    expect(serverSource).toContain('version: "1.5.1"');
    expect(dashboardSource).toContain("appInfo: { name: 'unity-dashboard', version: '1.5.1' }");
  });
